...
- [ ] T7: 실제 키움 일봉 연동 백테스트
  ...
  - 진행: STEP 07A 일봉 조회기, STEP 07B RESCUE, STEP 07C FACTOR fixture 구현·테스트 완료. STEP 07D에서 공식 `ka10081` 기준일 계약을 반영하고 42개 전체 테스트 및 독립 리뷰(`passed=true`)를 통과함; 커밋/push 대기. 상세는 `docs/AUTOMATION-STEP-07D-RESULTS.md`.
  - 남음: STEP 07D commit/push와 원격 SHA 확인, fixture 밖 실제 Kiwoom 응답·페이지네이션·가격부호·휴장일 동작 확인. ORB/BULL_FLAG 및 portfolio recommender는 규칙/계약 미정의로 fail-closed 상태이며 별도 설계 필요.
